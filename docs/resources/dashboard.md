...
| `schemaVersion` | number | Preset schema version. The current version is `3`. |
...
Each query carries `id`, `expr`, `dataType` (e.g. `metrics`), `editorMode`, and `step`. Set `editorMode = "builder"` — the create validator currently rejects `"code"`.
...
Any other value (for example `gauge`) is rejected with `Dashboard validation failed`. Only the `type` key is accepted inside `visualizationConfig`; extra keys (such as a nested `config` block) are also rejected.
