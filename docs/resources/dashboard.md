...
| `schemaVersion` | number | Preset schema version. |
...
Each query carries `id`, `expr`, `dataType` (e.g. `metrics`), `editorMode` (e.g. `builder`), and `step`.
...
Any other value (for example `gauge`) is rejected with `Dashboard validation failed`.
