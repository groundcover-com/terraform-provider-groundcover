// Package normalize holds the semantic normalization and comparison logic the
// groundcover Terraform provider uses to suppress false drift. It is a standalone
// package (not internal) so the upjet-generated Crossplane provider can reuse the
// exact same logic in its custom Observe path, keeping a single source of truth.
package normalize
...
// Exported wrappers for the duration helpers consumed across package boundaries
// (the v2 monitor resource and the Crossplane observe decorator). The internal
// implementations stay unexported so the in-package tests are unaffected.

// NormalizeHumanDurations converts human-readable durations ("10 minutes") to Go
// duration format ("10m"). See normalizeHumanDurations.
func NormalizeHumanDurations(s string) string { return normalizeHumanDurations(s) }

// NormalizeDayDurations converts day durations ("1d", "1d4h") to hours, since Go's
// time.ParseDuration does not understand the "d" unit. See normalizeDayDurations.
func NormalizeDayDurations(s string) string { return normalizeDayDurations(s) }

// NormalizeTimeString normalizes a single duration string to a canonical Go form
// ("30m0s" -> "30m"). See normalizeTimeString.
func NormalizeTimeString(s string) string { return normalizeTimeString(s) }
