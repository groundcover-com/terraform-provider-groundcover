...
* Fixed `groundcover_monitor_v2` create/update being rejected with `annotations key "_gc_monitor_v2_query_type" is reserved for internal use` since the backend started reserving the `_gc_` annotation prefix. The provider no longer writes that annotation — the query type is derived from the query shape on read (gcql sets `dataType`, metricsql requires `rollup`, raw_sql targets the `clickhouse` datasource); monitors created by older provider versions that still carry the annotation continue to be read correctly. Plan-time validation now rejects any user-configured `_gc_`-prefixed annotation key, and all `_gc_`-prefixed annotations returned by the API are filtered from state
...
* Fixed the same camelCase field loss on the inbound path: the user's `monitor_yaml` is now parsed with goccy/go-yaml as well, so camelCase keys in nested models (e.g. `autoComplete` in conditions) populate the request instead of being silently dropped before the request was even built — previously such fields never reached the API and caused a perpetual plan diff, since the stored monitor never contained them
* `monitor_yaml` field names must now match the documented camelCase schema exactly (e.g. `additionalFilter`, not `additionalfilter`) — the previous parser matched all-lowercase keys against Go field names while silently dropping the documented camelCase forms, so any lowercase spellings were already broken end-to-end
* `monitor_yaml` boolean fields now require `true`/`false` — YAML 1.1 literals (`yes`/`no`/`on`/`off`) that the previous parser coerced are rejected at apply with a clear unmarshal error pointing at the offending line
